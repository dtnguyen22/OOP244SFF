...
namespace aid {
	...
	public:
		...
		int quantity() const;
		bool operator>(const char*) const;
		bool operator>(const iGood&) const;//ms5
		int operator+=(int);
	};
	//helpers
	std::ostream& operator<<(std::ostream&, const iGood&);//ms5
	std::istream& operator>>(std::istream&, iGood&);//ms5
	double operator+=(double&, const iGood&); // ms5
